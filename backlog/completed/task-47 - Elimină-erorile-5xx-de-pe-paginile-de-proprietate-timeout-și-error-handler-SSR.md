...
2. **`src/server.ts` — error handler global**: middleware `(err, req, res, next)` după ruta catch-all: loghează eroarea (cu URL) și răspunde 503 (NU 500 generic) pentru erori de upstream/render. Atenție: 404-urile reale rămân pe fluxul TASK-115 (nu transforma totul în 503).
