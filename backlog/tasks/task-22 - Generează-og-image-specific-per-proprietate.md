...
2. **Strategic (SSR — deja în loc):** SSR-ul randează deja `og:image` pe server din `property.thumbnail` (mecanism implementat: `property-details` apelează `updatePageMeta`). Ce rămâne de fapt: (a) valoarea să fie un **URL public real**, nu `data:URI` base64 — depinde de TASK-3; (b) opțional, un compozit 1200×630 brand-wise (logo + prima poză + preț) generat la upload via `PhotoAdminService`.
