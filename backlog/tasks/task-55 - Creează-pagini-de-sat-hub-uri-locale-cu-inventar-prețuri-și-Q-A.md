...
2. **Conținutul unei pagini-sat** (≥600 cuvinte UNICE per sat — nu template umplut cu sinonime; regula 60%+ conținut unic):
   ...
   - **Anunțurile active din sat** (filtrare după localitate — necesită câmp `village`/`locality` pe proprietate, vezi TASK-54);
...
3. **Linking intern bidirecțional**: anunț → pagina satului („Despre Vaideeni") și pagina satului → anunțuri; homepage/footer → `/sate`. Link-uri REALE `<a href>` (TASK-58).
4. **Sitemap**: include `/sate` + cele 6 pagini (TASK-13 va prelua automat dacă generatorul citește rutele statice — adaugă-le explicit).
