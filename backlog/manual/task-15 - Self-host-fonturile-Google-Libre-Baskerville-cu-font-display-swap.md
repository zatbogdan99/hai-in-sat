...
NU se ruleaza prin pipeline. Motiv: pasul 1 cere **descarcarea fisierelor `.woff2` de pe internet** (google-webfonts-helper), iar agentii pipeline-ului ruleaza sandboxed, fara garantie de acces la retea — pasul de implement ar pica sau, mai rau, agentul ar inventa cai de fisiere inexistente.

**Daca vrei sa-l rulezi prin pipeline:** descarca intai tu subsetul latin + latin-ext pentru Libre Baskerville 400 / 700 / italic-400 in `src/assets/fonts/`, comite fisierele, si abia apoi taie un task de cod care face doar: `@font-face` in `styles.scss`, stergerea liniilor 170-172 din `index.html` (2× preconnect + stylesheet Google Fonts) si `<link rel="preload">` pe varianta 400.
...
6. Sinergie TASK-114 (CSP): după self-host, domeniile fonts.googleapis.com/fonts.gstatic.com se pot scoate din `style-src`/`font-src`.
