...
NU se ruleaza prin pipeline. Motiv: identic cu TASK-32 — AC#2 cere 3 articole × minimum 800 de cuvinte pe subiecte juridice si de piata locala (PUG/PUZ, costuri reale, profil de sat). Continut pe care agentii l-ar halucina. AC#5 (ritm editorial) e un angajament al owner-ului, nu o modificare de cod.

**Daca vrei sa recuperezi partea de cod pentru pipeline:** taie un task de framework — rutele `/articole` si `/articole/:slug`, parser Markdown cu front matter, citirea `.md`-urilor si pe server (SSR!), schema `Article`, intrarile in sitemap cu `lastmod` din front matter — livrat cu UN articol demo evident marcat ca placeholder. Scrierea articolelor reale ramane aici.
...
- `src/app/app.routes.ts` (rute noi — lazy, conform TASK-130)
- `src/assets/articles/*.md` + index-ul lor
- `package.json` (dependența de parser Markdown)
- `scripts/generate-sitemap.js` (include `/articole` + `/articole/:slug`, cu `lastmod` din front matter — sinergie TASK-129)
...
Cross-ref: NU e duplicat cu noul TASK-32 (pagini de sat /sate) - TASK-33 = blog/articole (/articole), TASK-32 = hub-uri de localitate. Tipuri de continut diferite; amandoua au nevoie de TASK-120 (link-uri interne crawlabile). TASK-32 e prioritizat mai sus in auditul nou.
<!-- SECTION:NOTES:END -->
